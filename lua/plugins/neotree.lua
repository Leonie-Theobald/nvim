				...
				filesystem = {
					...
					components = {
						git_status_or_default = function(config, node, state)
							...
							local index_git, worktree_git = normalize_git_status(git_status)

							if not i and not w then
							...
							elseif i == "!" then
								...
								-- modified unstaged (THIS FIXES [.M])
							elseif w == "M" then
								return { text = "Z", highlight = "NeoTreeGitUnstaged" }

								-- staged
							elseif i == "M" and w == " " then
